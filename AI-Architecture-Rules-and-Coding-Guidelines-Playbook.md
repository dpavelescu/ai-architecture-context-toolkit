...
# `covers` (optional) declares which concerns a source speaks to — a relevance hint so
# discovery selects by what a source covers, not by its filename. A bare path is fine too.
sources:                               # authoritative, human-owned artifacts
  sad:      [{ path: docs/architecture/sad.md, covers: [boundaries, integration, data-ownership] }]
  adrs:     [docs/architecture/adrs/]   # bare path — covers judged from the docs themselves
  specs:    [{ path: docs/contracts/openapi/, covers: [api-contract] }, docs/contracts/asyncapi/]
...
5. **Write the clean guidance `produce` selects** → the AI Architecture Rules (`docs/architecture/ai-architecture-rules.md`) and/or the AI Coding Guidelines (`docs/engineering/ai-coding-guidelines.md`) — **final rules only, no TBDs/placeholders/open-gaps sections**; everything unsettled went to the ledger. Lay each out in its standard sections (§5 "Generated file structure") — only sections with real content, omit the rest, don't pad; each rule a pointer to its source, prefer a canonical in-repo example, don't repeat content; the Guidelines link to the Architecture Rules, never restating them. On a refresh, merge into the existing file. **Validate against representative code** (classify each pattern: aligned / current-approved / target-ready / target-not-ready / brownfield exception / known legacy / suspected drift / ask-first) and add a Brownfield Guardrail to the Architecture Rules only for a misleading current-vs-target gap. *(Doc-only, no source access: skip validation and current-vs-target Guardrails — both need code; send ambiguity to the ledger rather than infer.)*
6. **Report:** propose the read order (source map → Architecture Rules → Coding Guidelines → ledger) for the repo's agent instruction file — a recommendation for a human made every run, since the toolkit only recommends and never edits that file. Emit the single Result below — clean files + ledger written together (unless discovery found neither sources nor code, in which case nothing is written and it says what's missing).
