...
**Procedure**
- With the **read-source-map** skill, bounded by `scope`, load the sources relevant to the
  guidance's concerns (architecture sources, specs, code), plus any existing `guidance` and the
  `ledger`. Search and read as often as it takes.
- If it resolved no `sources` — neither approved sources nor code — write nothing and stop; report
  what's missing.
- If it resolves existing `guidance` (Architecture Rules, Guidelines, or approved Guardrails) or a
  `ledger` → this run is a refresh: that guidance is the approved `baseline`. Per-learning evolution
  stays with `ai-guidance-update`.
...
**Procedure**
- Split every concern that matters into what the approved sources already settle and what still
  needs a human; the split decides what the clean files may say and what they must stay silent on.
...
- Put each `raise: live` candidate one at a time, most critical first, with its **proposal**, any
  competing reading the sources support, and the **evidence** behind them — saying where a proposal
  is code-derived, since that is lowest authority and settles nothing on its own.
- Offer those as suggestions, never as a closed set: the user may take one, edit it, answer in their
  own terms, or defer it to the ledger.
  - **Answered** (taken, edited, or answered in their own terms) → that answer is the decision: it
...
- Never guess a critical item, never silently defer one, and never start writing while a live
...
**Procedure**
- Record the still-undecided candidates in the ledger with the **update-clarifications-ledger**
  skill.
- Take its count line for the Result.
...
- For each current-vs-target divergence produce a Brownfield Guardrail with the
  **write-brownfield-guardrail** skill.
- For each file `produce` selects — the Architecture Rules, the Guidelines, or both — write it from
  the **final rules** with the **write-guidance-file** skill, merging into the existing file on a
  refresh.
...
**Procedure**
- Propose the read order for the repo's agent instruction file (conventionally `CLAUDE.md` or
  `AGENTS.md`): read the source map, Architecture Rules, Coding Guidelines, and clarifications
  ledger — in that order — before analysing, planning, coding, or reviewing.
